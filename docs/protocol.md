...
| | |                                                                                                                                                                                                                                    |
|---|---|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 状态 | 主协议保持自有 JSON 与信封 `v:1`；v0.60.2 发现/扫描可靠性及可选 dp1 关联探活（#317）                                                                                                                                                   |
| 日期 | 2026-09-18（#317） |
...
**批量开机风暴对策**（统一 VM 环境集中开机是常态，决议 #22）：① 应答抖动窗口按已知在线规模自适应——在线 <100 用 0–2s，每多 100 在线扩 1s，上限 0–8s；② 对 10s 内已互发过 `entry`/`alive` 的节点不重复应答；③ 入站 `entry` 处理排队削峰，处理不过来时丢弃靠 `presence` 周期自愈：未知节点心跳触发限频定向握手；不直接信任心跳中的身份。
...
- **按需探活（在线二次校验，#317）**：定向 `entry` 可携带可选 `probeId`（非空字符串，最长 64）；支持 `dp1` 的端在 `alive` 原样回传，绕过普通发现的随机延迟/10s 去重，每节点应答最短间隔 1s。打开会话向支持端探活，1s 重试一次、2s 内无有效活动则转离线；旧端兼容等待 20s，在 10s 时重试，覆盖原有去重与最多 8s 抖动。等待绑定 nodeId 与源 IP+端口，任何被接受的新活动可避免误判。
...
```
   ...
   收到 `peers` 后，对**陌生且 lastSeen < 10 分钟**的条目单播 `entry` 验证，**收到 `alive` 才入列表**（不直接信任转述，防列表投毒）。桥节点传播地址；每对通信节点仍须 UDP/TCP 直接可达，不中继数据，也不绕过 VLAN/ACL。`peers` 每包最多 8 条、同一目标相邻包至少间隔 50ms，避免撞上每来源 30 包/秒、突发 60 包的接收限速；重复交换请求合并，不积压多份完整列表。
...
- 入站校验：发送者必须已在线且源 IP+UDP 端口匹配节点表；只接受合法 IPv4 CIDR，且展开主机数不得超过 1024（最大 /22）；每包最多 10 个网段；未知字段忽略。
- 合并：收端只新增本机没有、且未被用户忽略的网段记录，并保存来源 nodeId / 显示名 / addedAt；远端不能删除、覆盖或修改本机已有记录。
- 扫描：收到新网段后进入本机后台扫描队列，首次随机延迟 30–90 分钟；每次完成后等待至少 12 小时并再次随机延迟 30–90 分钟；重启时尚未到期的任务补排到下次允许时间；在线节点数超过 50 时，按 `(self nodeId + cidr)` 的稳定 hash 抽样，默认约 10% 节点参与扫描，其他节点继续依赖 `peers` gossip 学到结果。
- 调度：全局、单段与后台扫描共用一个逐地址队列/定时器，手动任务优先，后台保留进度；手动间隔至少 8ms，后台至少 62ms；仅实际完成后写扫描时间，删除网段或退出时取消后台任务，执行前复核来源/抽样资格。
...
- 2026-09-18 决议 #317（应用 0.60.2）：发现丢包恢复、同版本资料乱序保护、dp1 定向探活、扫描统一调度与周期补排、网段来源校验和 gossip 错峰发送。信封 v:1 保持；probeId 为可选兼容字段。相同 profileRev 按同一发送端信封 ts 防旧包覆盖；同毫秒冲突/时钟回拨触发重新握手，匹配当前 probeId 的 alive 可确认该版本当前资料；确认时发生时钟回拨后，该版本后续变化仅接受关联应答，直到资料版本递增。旧端缺少关联应答时仅按时间戳尽力排序，完整恢复保证需双方升级。未知心跳/资料冲突握手每节点最短 10s、最多 1024 个在途请求，超时释放。
