...
| |                                                                                                                                                                                                              |
|---|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 状态 | v1.86；v0.60.2 同步与扫描可靠性修复（#317），目标机权限/性能待实测 |
| 日期 | 2026-09-18 |
...
- `app-state` 统一保存资料版本：取配置和运行中版本的较大值，资料或能力实际变化时递增并原子写回配置，再同步运行中对象。能力变更仅持久化版本，不将动态能力写成固定配置，也不改变首次向导状态；重复能力通知不写盘。
- `PeerRegistry.touch` 保持版本与源地址校验，对允许接收的完整资料按值比较；同版本的公司、部门、团队、头像、主机、平台、端口、应用版本及能力变化均触发 `updated`。沿用 200ms 界面推送和 1s 联系人落库；重复内容、普通心跳和聊天探活不增加刷新。
- 验证包含版本错开后的独立字段保存、能力变化与重启后的版本连续性、同版本资料与重复/旧报文，以及 `127.0.0.1`、`broadcastTargets: []` 下的发现报文回环。无协议字段、数据库结构或依赖变化。

- 2026-09-18 v1.85 决议 #316：修复资料版本错开与同版本非昵称字段刷新遗漏；应用 **0.60.1**。

### 发现与扫描可靠性（决议 #317）

- Discovery 统一扫描队列，逐地址调度，手动优先，完成/进度回调供全局 UI 与后台共用；停止/删除清理定时器和取消任务。
- services/range-scan.ts 编排远端 CIDR 的周期、持久化与执行时资格复核，保留 30–90 分钟错峰、12 小时间隔与大网抽样；定时器到期/任务完成后重新排期。主进程只装配和转发。
- PeerRegistry 保持资料版本单调，并按发送端时间戳抑制同版本旧包；Discovery 用可选 probeId 关联当前全量应答，解决同毫秒冲突/时钟回拨。dp1 快速探活使用独立应答节流，旧端等待去重与抖动窗口。
- RangeSync 校验已在线节点及实际 UDP 来源；gossip 同目标每 50ms 一包，按需取摘要，重复目标合并并在 stop 清理。无需新增依赖或数据库迁移。

- 2026-09-18 v1.86 决议 #317：发现自愈、资料排序、主动探活与扫描调度修复；应用 **0.60.2**。
