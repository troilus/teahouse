...
> 当前更新：2026-09-18，**v0.60.2 信息同步与扫描可靠性修复（#317）**。陌生心跳补握手、同版本资料乱序/时钟回拨保护、dp1 定向探活、扫描排队/续扫/周期补排、网段来源校验、gossip 分包错峰。`services/range-scan.ts` 编排后台周期；`Discovery` 统一所有逐地址扫描。下一决议 **#318**。本轮本地优化，尚未发布。
> 本轮验证：126 个测试文件 / 842 测试、Electron ABI 数据库、双端类型、构建与渲染体积门禁、隔离 smoke 全部通过。真实 Electron 22 验证资料/分组刷新、dp1 能力公布和全局/单段扫描 IPC/完成进度；回环含旧端延迟应答，模拟千节点和长期调度覆盖限速/续扫/补排。Win7/UOS/麒麟及真实 VLAN/千台终端仍须目标环境复测。
...
> 当前补充：2026-09-20（**本地分叉：端到端加密随上游 v0.60.2 合并保留**）：本 fork 在上游基线之上保留端到端加密扩展——X25519 密钥协商 + AES-256-GCM，覆盖单聊与群聊文本，双方声明 `e2e1` 才启用，上游原版节点自动明文兼容（不需要开关）。设计、威胁模型、协商时序与排障见 [e2e-encryption.md](e2e-encryption.md)。`e2e1` 能力位、`key-exchange` 报文、`peers.pub_key` / `messages.encrypted` 列属**本地协议与存储扩展**，上游 `protocol.md` / `tech-design.md` 不收录这些细节，以本文与代码为准。全部 `[e2e]` 调试日志**有意保留**（便于向上游提 PR 时复现与验证），正式发布前需改为按设置项开关。本次合并顺带加固：`PeerRegistry.touch` 在对端资料未携带 `pubKey` 时保留已知公钥，避免内存节点表的指纹展示被同版本空资料覆盖清空（数据库侧 `peers.pub_key` 另有"空值不覆盖"保护，加密决策读数据库，因此不会明文降级）。
> 合并后验证：`typecheck`、`test`（127 文件 / 862 用例）、`test:db`（Electron ABI 110 / node 16.17.1）、`build`（含渲染体积门禁，公共启动闭包 87,435 字节）、隔离 `smoke`、`check:docs`（13 组中英文文档）全部通过。Windows 上 `messenger` 用例曾因端口落在系统保留区间（`netsh int ipv4 show excludedportrange protocol=tcp`）出现 `listen EACCES`，属环境问题，必要时排除该区间后重跑。
